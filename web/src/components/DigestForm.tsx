...
import { type DigestSuccess, postDigest } from '../api/digestApi';
...
export default function DigestForm({ defaultTxnRef = '' }: Props) {
    ...
    return (
      <div style={s.card}>
        <h2 style={{ margin: '0 0 1.5rem', fontSize: 20, color: '#111827' }}>
          Billing Activity Digest
        </h2>
        <div style={s.successBanner}>
          <div style={{ fontWeight: 700, fontSize: 15, color: '#15803d', marginBottom: '0.25rem' }}>
            Digest posted to Slack
          </div>
          <div style={{ fontSize: 12, color: '#6b7280', marginBottom: '1rem' }}>
            Last {result.windowDays} days · {result.consultantId}
          </div>

          <div style={s.statsGrid}>
            <div style={s.statCard('#86efac')}>
              <div style={s.statValue}>{result.activeCount}</div>
              <div style={s.statLabel}>Active subs</div>
            </div>
            <div style={s.statCard('#a5b4fc')}>
              <div style={s.statValue}>{result.mrr}</div>
              <div style={s.statLabel}>MRR</div>
            </div>
            <div style={s.statCard('#93c5fd')}>
              <div style={s.statValue}>{result.newSignups}</div>
              <div style={s.statLabel}>New signups</div>
            </div>
            <div style={s.statCard('#fca5a5')}>
              <div style={s.statValue}>{result.churn}</div>
              <div style={s.statLabel}>Churn</div>
            </div>
          </div>
          ...
          {result.overdueInvoices > 0 && (
            <div style={{
              ...
              {result.overdueInvoices} overdue invoice{result.overdueInvoices !== 1 ? 's' : ''} — follow up with clients
            </div>
          )}
  ...
  return (
    <div style={s.card}>
      <h2 style={{ margin: '0 0 0.25rem', fontSize: 20, color: '#111827' }}>
        Billing Activity Digest
      </h2>
      <p style={{ margin: '0 0 1.5rem', fontSize: 13, color: '#6b7280' }}>
        Fetches site-wide stats and posts a summary to the transaction's Slack channel.
      </p>
      ...
      <form id={formId} onSubmit={(e) => void handleSubmit(e)} noValidate>
        ...
        <div style={s.field}>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setFieldErrors((f) => { const n = { ...f }; delete n.txnRef; return n; });
            }}
            style={{
              ...s.input,
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setFieldErrors((f) => { const n = { ...f }; delete n.adminPassword; return n; });
                }}
                style={{
                  ...s.input,
        ...
        <button type="submit" disabled={loading} style={s.btn(loading)}>
          {loading ? 'Fetching digest…' : 'Run billing digest'}
        </button>
      </form>
      ...
      {errorMsg && !authError && !expiredMsg && (
        <div style={s.failBanner}>
          <div style={{ fontWeight: 700, color: '#854d0e', marginBottom: 6 }}>
            Maxio error — digest not fetched
          </div>
